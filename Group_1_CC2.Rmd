...
1. **Ella** — 20457793
    + [BIOL343_CC](https://github.com/Ella-Sha/biology343)
1. 
...
------|---------|------
population_type | A-asexual, S-sexual | N/A
ovary_width     | The width of the ovary, which is a reliable measurement for flower size | ${mm}$
ovary_height    | The height of the ovary, which is a reliable measurement for flower size | ${mm}$
flower_number | The average number of flowers produced by each individual in a sample of plants from each population | counts
...
```{r import-clean}
cc1_data <- read.csv("~/Desktop/343 Data analysis/biology343/data/A1_Data_Excel.csv",header=T)
...
The units for the equation are: $width\ (\text{mm}) \times height\ (\text{mm}) = area\ (\text{mm}^2)$.
...
cc1_data$ovary_area <- cc1_data$Ovary.Width..mm. * cc1_data$Ovary.Height..mm.
area_means <- aggregate( ovary_area ~ Population.Type, data = cc1_data, mean)
...
kable(area_means, caption = "Distribution of ovary area by population type")
```
...
library(ggplot2)
ggplot(aes(x = ovary_area),data=cc1_data) + geom_histogram(binwidth= 1) + facet_wrap(.~Population.Type) + xlab("Population Type")+ ylab("Mean ovary Area(mm^2)") + labs(title = "Histogram of mean ovary area for sexual and asexual population")+  theme_classic()
...
![Linked .png file](./images/Hydroponic_g13.jpg) 
